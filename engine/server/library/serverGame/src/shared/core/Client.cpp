        ...
        switch (messageType) {
            ...
            case constcrc("AirspeederCrash") : {
                ...
                if (creatureOwner && creatureOwner->isAuthoritative()) {
                    ...
                    scriptParams.addParam(Unicode::narrowToWide(msg.getValue()));
                    scriptParams.addParam(0.0f);
                    creatureOwner->getScriptObject()->callScriptCommandHandler("onAirspeederCrash", scriptParams);
                }
                break;
            }
            ...
            case constcrc("AutoPilotWaypoint") : {
                ...
                if (creatureOwner && creatureOwner->isAuthoritative() && creatureOwner->getScriptObject()) {
                    ScriptParams params;
                    params.addParam(msg.getValue().first, "x");
                    params.addParam(msg.getValue().second, "z");
                    ScriptDictionaryPtr dictionary;
                    GameScriptObject::makeScriptDictionary(params, dictionary);
                    if (dictionary.get() != nullptr) {
                        dictionary->serialize();
                        creatureOwner->getScriptObject()->handleMessage("handleAutoPilotWaypoint", dictionary);
                    }
                }
                break;
            }
